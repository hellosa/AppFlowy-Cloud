...
pub async fn get_page_view_collab_noauth(
  pg_pool: &PgPool,
  collab_access_control_storage: &CollabAccessControlStorage,
  workspace_id: Uuid,
  view_id: Uuid,
  ...
  let page_collab_data = match view.layout {
    collab_folder::ViewLayout::Document => {
      get_page_collab_data_for_document_noauth(collab_access_control_storage, workspace_id, view_id)
        .await
    },
    collab_folder::ViewLayout::Grid
    | collab_folder::ViewLayout::Board
    | collab_folder::ViewLayout::Calendar => {
      get_page_collab_data_for_database_noauth(
        pg_pool,
        collab_access_control_storage,
        workspace_id,
        view_id,
...
async fn get_page_collab_data_for_document_noauth(
  collab_access_control_storage: &CollabAccessControlStorage,
  workspace_id: Uuid,
  view_id: Uuid,
) -> Result<PageCollabData, AppError> {
  let doc = get_latest_collab_encoded(
    collab_access_control_storage,
    GetCollabOrigin::Server,
    workspace_id,
    view_id,
  ...
  let doc_data = serde_json::from_slice(&doc_state).map_err(|err| {
    AppError::Internal(anyhow::anyhow!(
  ...
  let page_data = PageCollabData::Document(doc_data);

  Ok(page_data)
}

async fn get_page_collab_data_for_database_noauth(
  pg_pool: &PgPool,
  collab_access_control_storage: &CollabAccessControlStorage,
  workspace_id: Uuid,
  view_id: Uuid,
  ...
  let queries: Vec<QueryCollab> = row_ids
    ...
    .collect();
  let rows = batch_get_latest_collab_encoded(
    collab_access_control_storage,
    GetCollabOrigin::Server,
    workspace_id,
    queries,
  ...
  .map_err(|err| {
    AppError::Internal(anyhow::anyhow!(
      "Unable to get database row collabs: {}",
      err
    ))
  })?;
  let rows_map: HashMap<String, EncodedCollab> = row_ids
    .iter()
    .zip(rows.into_iter())
  ...
  let data = serde_json::to_value(&database_data).map_err(|err| {
    AppError::Internal(anyhow::anyhow!(
      "Failed to encode database body: {}",
      err
    ))
  })?;

  let page_data = PageCollabData::Database { data, rows: rows_map.into() };
  Ok(page_data)
}
